...
public class StatsClientImpl implements StatClient {
    ...
    private final String serviceName = "STATS-SERVER";
    ...
    private URI getUri(String serviceName) {
        ServiceInstance instanceInfo = null;
        try {
            instanceInfo = discoveryClient.getInstances(serviceName).getFirst();
        } catch (Exception e) {
            URI uri = UriComponentsBuilder.fromUriString("http://localhost:8080").build().toUri();
            return uri;
        }
        log.info("Отправлено сообщение на сервер статистики  URI = {}", instanceInfo.getUri());
